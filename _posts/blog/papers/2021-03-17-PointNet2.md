...
As discussed earlier, it is common that a point set comes with nonuniform density in different areas.   
...
> Fig. 3 (a)에서 보듯이, multiscale patterns을 캡처하는 간단하지만 효과적인 방법은 서로 다른 scales의 그룹화 layers를 적용한 후 PointNets에 따라 각 scale의 features을 추출하는 것이다.
