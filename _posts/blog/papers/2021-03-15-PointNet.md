...
> - 우리는 이러한 net이 33D shape classification, shape part segmentation 및 scene semantic parsing tasks을 수행하기 위해 어떻게 훈련될 수 있는지 보여준다.
> - 방법의 안정성 및 효율성에 대한 철저한 경험적, 이론적 분석을 제공합니다.
> - 선택된 neuron들이 net에서 계산한 3D features을 설명하고 성능에 대한 직관적인 설명을 개발한다.
...
또한 로컬 기능과 글로벌 기능으로 분류할 수 있습니다. 특정 작업의 경우 최적의 형상 조합을 찾는 것이 사소한 일이 아니다.
...
하지만, 이러한 방법들은 현재 organic objects과 같은 manifold meshes에 제한되고 있으며, 어떻게 그것들을 가구와 같은 non-isometric shapes으로 확장시킬지는 분명하지 않다.
...
Formally, let  
$$\mathcal{X} = \{S:S \subseteq [0,1]^m \} and \|S\| = n, f : \mathcal{X} \rightarrow \mathbb{R}$$ is a continuous set function on X w.r.t to Hausdorff distance $$d_H(., .)$$, i.e., $$\forall\epsilon > 0, \exists\delta > 0, $$ for any $$S,S' \in \mathcal{X}, if d_H(S,S') < \delta, then \|f(S) - f'(S)\| < \epsilon $$
