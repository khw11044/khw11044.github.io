...
> $$N$$ Points의 point cloud : a set $$P = \{p_1, ... , p_N\}$$  
...
보다 구체적으로, 우리는 각 vertex의 $$r_0$$ radius 내에서 raw points를 검색하고 세트의 신경망을 사용하여 features을 추출한다.  
...
> $$\Delta x_i^t$$는 정점들이 좌표를 등록하기 위한 조정 offset이다.  
$$h^t(.)$$는 이전 반복의 center vertex state value을 사용하여 offset을 계산한다.  
...
vertex이 객체의 경계 상자 내에 있으면 객체 클래스를 정점에 할당한다.  
정점이 경계 상자 외부에 있는 경우 배경 클래스를 여기에 할당한다.  
...
또한 confidence score를 조합 교차점(Intersection-of-Union:IoU) factor와 occlusion factor로 weighted된 classification scores의 합으로 계산한다.  
폐색 계수(occlusion factor)는 점유 volume ratio을 나타낸다.  
...
> $$(l_m, h_m, w_m)$$를 Car bounding boxes의 중간 크기(3.88m, 1.5m, 1.63m)로 설정했다.  
우리는 $$\theta \in [-\pi/4, \pi/4]$$가 있는 side-view car와 front-view car $$\theta \in [\pi/4, 3\pi/4]$$을 두 가지 다른 클래스로 취급한다.  
따라서 우리는 각각  $$\theta_0 = 0$$ 및 $$\theta_0 = \pi/2$$를 설정한다.  
scale $$\theta_m$$는 $$\pi/2$$로 설정된다.  
_Background_ 클래스 및 _DoNotCare_ 클래스와 함께 4개의 클래스가 예측된다.  
우리는 $$r = 4m$$ 및 $$r_0 = 1m$$로 graph를 구성한다.  
training에서 0.8m의 voxel size와 inference에서 0.4m의 voxel size로 downsample된 point cloud를 $$\hat{P}$$으로 설정.
